...
/**
 * Created by Santosh.Kumar on 9/28/2016.
 */
public class InitShutdownController {

    /**
     * preferred repository selector config param. Maps to web.xml
     * &lt;context-param&gt; with
    ...
    /**
     * relative path to config file within current webapp config param. Maps to
     * web.xml &lt;context-param&gt; with
     * &lt;param-name&gt;log4j-config&lt;/param-name&gt;
     */
    public static final String PARAM_LOG4J_CONFIG_PATH = "log4j-config";

    /**
     * config file re-reading specified in milliseconds config param. Maps to
     * web.xml &lt;context-param&gt; with
    ...
    /**
     * path to be read from a log4j xml config file as a system property config
     * param. Maps to web.xml &lt;context-param&gt; with
     * &lt;param-name&gt;log4j-log-home&lt;/param-name&gt;
     */
    public static final String PARAM_LOG4J_LOG_HOME = "log4j-log-home";

    /**
     * system property to be used in a log4j xml config file config param. Maps
     * to web.xml &lt;context-param&gt; with
    ...
    /**
     * Log4j specific initialization. Sets up log4j for the current servlet
     * context. Installs a custom repository selector if one hasn't already been
     * installed.
     *
     * @param context the current servlet context
     */
    public static void initializeLog4j(final ServletContext context) {
        ...
        try {
            ...
            //in WEB-INF/lib, but not this library (assuming they aren't both in log4j.jar).
        ...
        try {
            ...
            if (savedClassLoader != null) {
                ...
                //loader now that our purpose (avoiding class cast exceptions) has been v
     ...
     * 'logj4-config'. The path is assumed to be relative to the current
     * context. If set, the path is normalized with any preceeding slash removed
     * as the other methods expect the path to be formatted this way.
     *
     ...
     *
     * @param configPath the location of the log4j configuration file relative to the context path
     * @param context    the current servlet context
     */
    private static void configureLog4jFromURL(final String configPath, final ServletContext context) {
        ...
        //configureAndWatch() here because that requires an absolute system file path.
     ...
     *
     * @param configPath the fully qualified system path location of the log4j configuration file
     * @param context    the current servlet context
     */
    private static void configureLog4jFromFile(final String configPath, final ServletContext context) {
        ...
        boolean isXMLConfigFile = configPath.endsWith(".xml");
        File log4jFile = new ConfigurationLoaderWebapp(context).getFile(configPath);
        //Now let's check if the given configPath actually exists.
        if (log4jFile.canRead()) {
            long timerIntervalVal = getTimerIntervalFromContext(context);
            context.log("Configuring Log4j from File: " + log4jFile);

            if (timerIntervalVal > 0) {
                context.log("Configuring Log4j with watch interval: " + timerIntervalVal + "ms");

                if (isXMLConfigFile) {
                    DOMConfigurator.configureAndWatch(log4jFile.getAbsolutePath(), timerIntervalVal);
                } else {
                    PropertyConfigurator.configureAndWatch(log4jFile.getAbsolutePath(), timerIntervalVal);
                }
            } else {
                if (isXMLConfigFile) {
            ...
            // let Log4j look for the default files (log4j.properties or log4j.xml) on its own.
            displayConfigNotFoundMessage();
        }
    }
     ...
     * It does not guarantee that your preferred selector will be used. If some
     * other code sets the selector first, that is the selector that all
